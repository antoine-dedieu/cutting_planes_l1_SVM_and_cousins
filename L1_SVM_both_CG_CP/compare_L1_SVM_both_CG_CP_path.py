...
#---Results
	...
	times_SVM_CG               = [[ [] for i in range(n_alpha_list)] for P in N_P_list]
	times_SVM_both_CG_CP       = [[ [] for i in range(n_alpha_list)] for P in N_P_list]  #delete the columns not in the support

	objvals_SVM_CG              = [[ [] for i in range(n_alpha_list)] for P in N_P_list]
	objvals_SVM_both_CG_CP      = [[ [] for i in range(n_alpha_list)] for P in N_P_list] 
	...
	for N,P in N_P_list:
		...
		#---Warm start Gurobi
			index_SVM_CG_liblinear, time_liblinear_CG = liblinear_for_CG('squared_hinge_l1', X_train, y_train, alpha_list[0], f)
			
			index_samples_liblinear, index_columns_liblinear, time_liblinear_both_CG_CP = liblinear_for_both_CG_CP('squared_hinge_l1', X_train, y_train, alpha_list[0], f)


			model_L1_SVM           = 0
			model_SVM_CG           = 0
			model_SVM_both_CG_CP   = 0
			...
			#---L1 SVM with CG and not deleting
				...
				beta_SVM, support_SVM, time_SVM_CG, model_SVM_CG, _, objval_SVM_CG = L1_SVM_CG(X_train, y_train, index_columns_liblinear, alpha, epsilon_RC, time_limit, model_SVM_CG, False, f)
				times_SVM_CG[aux][aux_alpha].append((time_liblinear_CG+time_SVM_CG)/N)
				objvals_SVM_CG[aux][aux_alpha].append(objval_SVM_CG/objval_L1_SVM)


			#---L1 SVM with CG and not deleting
				write_and_print('\n\n###### L1 SVM with CG CP and liblinear #####', f)
				beta_SVM, support_SVM, time_SVM_add_delete, model_SVM_both_CG_CP, index_samples_liblinear, index_columns_liblinear, _, objval_both_CG_CP = L1_SVM_both_CG_CP(X_train, y_train, index_samples_liblinear, index_columns_liblinear, alpha, epsilon_RC, time_limit, model_SVM_both_CG_CP, delete_samples, f)
				times_SVM_both_CG_CP[aux][aux_alpha].append((time_liblinear_both_CG_CP+time_SVM_add_delete)/N)
				objvals_SVM_both_CG_CP[aux][aux_alpha].append(objval_both_CG_CP/objval_L1_SVM)
...
#---TIMES
	...
	times_SVM_CG_averaged_CG           = [ [ np.sum([times_SVM_CG[NP][i][loop]              for i in range(n_alpha_list) ]) for loop in range(loop_repeat)] for NP in range(len(N_P_list))]
	times_SVM_CG_averaged_both_CG_CP   = [ [ np.sum([times_SVM_both_CG_CP[NP][i][loop]      for i in range(n_alpha_list) ]) for loop in range(loop_repeat)] for NP in range(len(N_P_list))]


	times_L1_SVM_both_CG_CP_plots_path(type_Sigma, N_P_list, k0, rho, tau_SNR, times_L1_SVM_averaged, times_SVM_CG_averaged_CG, times_SVM_CG_averaged_both_CG_CP)
	plt.savefig(pathname+'/compare_times.png')
	plt.close()


#---RATIO
	objvals_SVM_CG_averaged              = [ [ np.mean([objvals_SVM_CG[NP][i][loop]              for i in range(n_alpha_list) ]) for loop in range(loop_repeat)] for NP in range(len(N_P_list))]
	objvals_SVM_CG_both_CG_CP_averaged   = [ [ np.mean([objvals_SVM_both_CG_CP[NP][i][loop]      for i in range(n_alpha_list) ]) for loop in range(loop_repeat)] for NP in range(len(N_P_list))]

	objval_L1_SVM_both_CG_CP_plots_path(type_Sigma, N_P_list, k0, rho, tau_SNR, objvals_SVM_CG_averaged, objvals_SVM_CG_both_CG_CP_averaged)
