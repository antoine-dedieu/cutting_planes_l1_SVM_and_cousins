...
def L1_SVM_CP_model(X, y, idx_CP, alpha, time_limit, model, f):
...
def add_constraints_L1_SVM(X, y, L1_SVM_CP, violated_constraints, idx_columns):
# Add a column in a model with respective constraints
    
    N,P = X.shape
    beta_plus   = np.array([L1_SVM_CP.getVarByName("beta_+_"+str(idx)) for idx in idx_columns])
    beta_minus  = np.array([L1_SVM_CP.getVarByName("beta_-_"+str(idx)) for idx in idx_columns])
    b0          = L1_SVM_CP.getVarByName("b0")
    ...
    #---Add associated constraint
        L1_SVM_CP.addConstr(  xi_violated + y[violated_constraint]*(b0 + quicksum([ X[violated_constraint][idx_columns[k]]*(beta_plus[k] - beta_minus[k]) for k in range(len(idx_columns))]))>= 1, 
                                 name="slack_"+str(violated_constraint))
        L1_SVM_CP.update()
