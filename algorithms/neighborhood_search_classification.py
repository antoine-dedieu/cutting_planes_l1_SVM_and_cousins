...
def randomized_NS(number_NS, type_loss, type_penalization, X, y, betas_grid, train_errors, original_train_errors, K0_list, number_decreases, alpha_list, X_add, L, epsilon, f, time_limit=60):    
...
#---MAIN LOOP
    ...
    for K0 in K0_list_NS:
        ...
        #---Change K
            ...
            beta_1, beta0_1, train_error_1 = algorithm1_unified(type_loss, type_penalization, X, y, K0, alpha, (start1, start0_1), X_add, L, epsilon, time_limit)   
            ...
            beta_3, beta0_3, train_error_3 = algorithm1_unified(type_loss, type_penalization, X, y, K0, alpha, (start3, start0_3), X_add, L, epsilon, time_limit)   
        ...
        #---Change alpha
            ...
            beta_2, beta0_2, train_error_2 = algorithm1_unified(type_loss, type_penalization, X, y, K0, alpha, (start2, start0_2), X_add, L, epsilon, time_limit)   
            ...
            beta_4, beta0_4, train_error_4 = algorithm1_unified(type_loss, type_penalization, X, y, K0, alpha, (start4, start0_4), X_add, L, epsilon, time_limit)   
        ...
        write_and_print('For K0='+str(K0)+',    Max improvement in % = '+str(round(100*np.max(ratio_improvements[K0]),4))+',    Mean improvement in % = '+str(round(100*np.mean(ratio_improvements[K0]),4))  ,f)



    #UNDERSTAND THE TREND
    write_and_print('\n\nStay             : '+str(move_from[0]), f)
    write_and_print('Move from K0-1   : '+str(move_from[1]), f)
    write_and_print('Move from alpha-1  : '+str(move_from[2]), f)
    write_and_print('Move from K0+1   : '+str(move_from[3]), f)
    write_and_print('Move from alpha+1   : '+str(move_from[4]), f)

    write_and_print('\nTime : '+str(time.time()-start), f)

    return new_betas_grid, new_train_errors
