...
def Matrix_different_support(betas_grid, K0_list, number_decreases):
...
#FOR EVERY POINT I THE GRID, RETURN THE POINT ON THE RIGHT AND ON THE LEFT WITH DIFFERENT SUPPORT 
    ...
    for K0 in K0_list: 
        previous_index = 0
        support1 = np.where(betas_grid[K0][0][0]!=0)[0]
        support2 = np.where(betas_grid[K0][0][0]!=0)[0]


        next_index=0
        while(len(set(support1)-set(support2))==0 and len(set(support2)-set(support1))==0 and next_index+1<number_decreases):
            next_index += 1
            support2 = np.where(betas_grid[K0][next_index][0]!=0)[0]
        matrix_different_support[K0].append((previous_index,next_index%(number_decreases-1)))


        #LOOP
        for loop in range(1,number_decreases):
            if(loop<next_index):
                matrix_different_support[K0].append((previous_index,next_index%(number_decreases-1)))

            #loop=number_decreases
            elif(loop+1==number_decreases):
                matrix_different_support[K0].append((previous_index,0))
                
            else:
                previous_index = loop-1
                support1 = np.where(betas_grid[K0][loop][0]!=0)[0]
                support2 = np.where(betas_grid[K0][loop][0]!=0)[0]
