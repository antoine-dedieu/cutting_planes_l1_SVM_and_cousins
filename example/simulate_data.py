...
#################### CASE 1: Sigma_ij = rho^(i-j) ####################
	...
	### First half
		...
		X[:N/2,:] = np.dot(L, u_plus).T + mu_positive

	### Second half
		u_minus   = np.random.normal(size=(P,N/2))
		X[N/2:,:] = np.dot(L, u_minus).T + mu_negative
		y[N/2:]   = -np.ones(N/2)
...
#################### CASE 2: Sigma_ij = rho ####################
	...
	### First half
		...
		X[:N/2,:] = np.concatenate([np.random.normal(loc=  1./float(math.sqrt(1-rho))*mu_positive[:k0], size=(N/2,k0)), np.random.normal(size=(N/2,P-k0))], axis=1)
		
		X[:N/2,:] = math.sqrt(rho)*X0_plus + math.sqrt(1-rho)*X[:N/2,:]

	### Second half
		X0_minus   = np.random.normal(size=(N/2,1))
		X[N/2:,:]  = np.concatenate([np.random.normal(loc= 1./float(math.sqrt(1-rho))*mu_negative[:k0], size=(N/2,k0)), np.random.normal(size=(N/2,P-k0))], axis=1)        

		X[N/2:,:] = math.sqrt(rho)*X0_minus + math.sqrt(1-rho)*X[N/2:,:]        
		y[N/2:]   = -np.ones(N/2)
