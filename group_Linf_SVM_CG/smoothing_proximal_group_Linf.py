            ...
            ### Is the group in the active set of groups ?
                ...
                ### Compute gradient
                    ...
                    #---If block CD then only need gradient on group
                        if type_penalization == 'l1_linf_CD':
                            gradient_loss = -0.5*np.dot(X_add[:, group_to_feat[idx]+[P]].T, gradient_aux)
                        else:
                ...
                ### Gradient step + Thresholding operators
                    if type_penalization == 'l1_linf_CD':
                        old_beta_m_group  = np.copy(beta_m[group_to_feat[idx]+[P]])
                        grad              = beta_m[group_to_feat[idx]+[P]] - 1./Lipchtiz_coeff[idx] * gradient_loss
                    ...
                    #---Residual updates
                        delta_beta_group   = np.dot(X_add[:, group_to_feat[idx]+[P]], beta_m[group_to_feat[idx]+[P]]-old_beta_m_group)
